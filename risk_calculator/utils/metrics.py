...
def calculate_metrics(params, btc_prices, vol_heston):
    ...
    target_d1 = (np.log(target_s / params['IssuePrice']) + (params['risk_free_rate'] + params['delta'] + 0.5 * np.mean(vol_last) ** 2) * params['t']) / (np.mean(vol_last) * np.sqrt(params['t']))
    target_d2 = target_d1 - np.mean(vol_last) * np.sqrt(params['t'])
    ...
    target_roe = params['risk_free_rate'] + params['beta_ROE'] * (params['expected_return_btc'] - params['risk_free_rate']) * (1 + np.mean(vol_last) / params['long_run_volatility'])
    ...
    distribution_metrics = {
        ...
        'expected_shortfall': np.mean(final_btc_prices[final_btc_prices <= np.percentile(final_btc_prices, 5)]),
        'price_distribution': {
